define(["exports"], function (exports) {
  ...
  function printCleanupDetails(deletedURLs) {
    ...
    if (deletionCount > 0) {
      logger.groupCollapsed(
        `During precaching cleanup, ` + `${deletionCount} cached ` +
  ...
  class StrategyHandler {
    ...
    async fetch(input) {
      ...
      if (
        request.mode === "navigate" && event instanceof FetchEvent &&
  ...
  class PrecacheStrategy extends Strategy {
    ...
    async _handleFetch(request, handler) {
      ...
      if (this._fallbackToNetwork) {
        ...
        const noIntegrityConflict = !integrityInRequest ||
        ...
        // See https://github.com/GoogleChrome/workbox/issues/3096
        if (
          integrityInManifest && noIntegrityConflict &&
    ...
    _useDefaultCacheabilityPluginIfNeeded() {
      ...
      if (cacheWillUpdatePluginCount === 0) {
        ...
        cacheWillUpdatePluginCount > 1 && defaultPluginIndex !== null
      ) {
    ...
    constructor(precacheController, options) {
      const match = ({
        ...
        for (const possibleURL of generateURLVariations(request.url, options)) {
          ...
          if (cacheKey) {
            const integrity = precacheController.getIntegrityForCacheKey(
              cacheKey,
            );
            return {
              cacheKey,
